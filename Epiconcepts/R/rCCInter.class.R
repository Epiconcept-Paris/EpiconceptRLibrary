...
setMethod("initialize", "rCCInter",
          function(.Object, df, x, exposure, by)
          {
            ...
            if (length(exposure) < 1) {
             ...
             .Col1Label = sprintf("rCCInter %s / %s", x, exposure);
            ...
            for (i in 1:0) {
              VALBY <- df[,by] == i;
              T = table(df[VALBY, exposure], df[VALBY, x])
              #print(T)
...
# -----------------------------------------------------------------------------
# function: rCCInter (call real constructor)
# Return: an object of type rCCInter
# -----------------------------------------------------------------------------
rCCInter <- function(df, x, exposure="", by="")
{
  return(new("rCCInter", df=df, x=x, exposure=exposure, by=by));
}

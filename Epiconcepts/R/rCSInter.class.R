...
setMethod("initialize", "rCSInter",
          function(.Object, df, x, exposure, by)
          {
            ...
            VALBY <- df[,by] == 1;
            T1 = table(df[VALBY, exposure], df[VALBY, x])
...
#                               "Prev. frac. ex.", "Prev. frac. pop", "chi2(1)", "Pr>chi2")
            ...
            # MISSING -----------------
            MIS_TO = nrow(df) - (BY1_TO + BY0_TO);
            MIS_PC = sprintf("%3.2f%s", (MIS_TO / nrow(df))*100, '%');
            
            # ==== Whitout by
            TG = table(df[, exposure], df[, x])
            C = rr(TG)
            V_RR = C[1];
            ...
            T = mht <- table(df[,x], df[,exposure], df[,by]);
...
# method show
# -----------------------------------------------------------------------------
setMethod("show" ,"rCSInter" ,
  function(object) {
    align  =  c("l","r","r","r","r","r","r","r","r");
    ec.xtable(object@rCSInter, align=align);
  }
)

# -----------------------------------------------------------------------------
# function: rCSInter (call real constructor)
# Return: an object of type rCSInter
# -----------------------------------------------------------------------------
rCSInter <- function(df, x, exposure="", by="")
{
  return(new("rCSInter", df=df, x=x, exposure=exposure, by=by));
}
